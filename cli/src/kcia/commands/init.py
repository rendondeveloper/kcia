...
GITIGNORE_ENTRIES = (
    ...
    ".cursor/mcp.json",
    # Regenerated by `kcia mcp add`; may carry MCP server headers.
    "opencode.json",
)
...
def _write_adapters(repo_root: Path, registry, entries: list[dict]) -> list[Path]:
    ...
    for index, entry in enumerate(entries, start=1):
        ...
        bundle = f".ai/generated/profiles/{resolved.id}/**"
        extra = (resolved.adapters.get("opencode") or {}).get("instructions") or []
        for item in [bundle, *extra]:
            if item not in instructions:
                instructions.append(item)

    written.extend(_write_opencode_json(repo_root, templates, instructions))
    return written


def _write_opencode_json(
    repo_root: Path, templates: Path, instructions: list[str]
) -> list[Path]:
    """Render repo-root `opencode.json`, preserving any existing mcp/extra keys."""
    ...
    if path.is_file():
        try:
            ...
            existing = {}
    mcp = existing.get("mcp") if isinstance(existing.get("mcp"), dict) else {}
