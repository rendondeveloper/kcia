...
configured agents can actually run, and whether the repository is initialized. Run it right
after installing, and again whenever anything looks wrong.
...
| Want | Command |
|---|---|
| install somewhere else | `KCIA_HOME=~/src/kcia KCIA_BIN=~/bin curl -fsSL .../install.sh \| bash` |
| update later | `~/tools/kcia/scripts/install.sh update` |
| uninstall | `~/tools/kcia/scripts/install.sh uninstall` |

**Verify**

```bash
kcia --version        # e.g. kcia 0.0.1
...
`AGENTS.md`, `.cursor/rules/*.mdc`), and adds all generated paths to `.gitignore`. Run it
again after updating kcia — it only rewrites what changed (`Already up to date` when nothing
moved).
...
| Flag | Effect |
|---|---|
| `--yes` | non-interactive: no questions at all (CI, scripts) |
| `--no-gitignore` | do not touch `.gitignore`; you manage ignore rules yourself |
...
Optional. Enable an MCP server per repository and kcia declares it and hands it to the right
agent; the **provider CLI owns the login** — kcia never stores credentials.
